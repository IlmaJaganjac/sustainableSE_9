...
The results reveal notable variations in energy consumption, response time, and power usage across different search engines, with some displaying consistently higher resource demands than others. Startpage stands out as the search engine with the longest response times and the highest energy delay product (EDP) across all weighting factors. This suggests that Startpage may have additional processing steps or a delayed query execution mechanism, potentially due to its privacy-focused architecture. Ecosia, on the other hand, demonstrates a steep increase in power usage during the first few seconds of a query, indicating that its search process may involve more intensive initial computations compared to alternatives. Additionally, scatter plots show that while Ecosia's total energy increases steadily with time, OceanHero maintains a more consistent energy level, implying differences in query handling efficiency across platforms.

Another key observation is the relationship between response time and energy efficiency. Most search engines experience a decrease in power consumption as duration increases, but Startpage follows the opposite trend. Its power usage rises sharply after six seconds, suggesting that it may require additional post-processing or backend retrieval operations. Violin plots further highlight that Ecosia, Bing, and You.com exhibit greater variability in power consumption, whereas Google and Mojeek maintain more stable energy profiles. Despite Google's reputation for data-heavy operations, it appears relatively efficient in terms of total energy consumption, ranking among the lowest alongside Mojeek. However, You.com emerges as the most energy-demanding search engine overall, followed by Bing, both of which consume significantly more energy per query than the rest. This suggests that while some search engines optimize response time, others may trade efficiency for enhanced search features or user interface rendering.

Comparative analysis across engines reinforces these trends, particularly in percentage change comparisons of power and total energy. Search engines compared against Google and Startpage generally show significant differences in energy consumption, indicating that these two operate with distinct efficiencies. The highest percentage changes appear when comparing You.com with other engines, further emphasizing its energy-intensive nature. Memory usage trends also highlight Ecosia’s pronounced peak early in the search process, while Startpage displays a delayed but sharp increase after six seconds, reinforcing the idea that some engines execute additional processes either at the beginning or end of a search. These findings suggest that the sustainability of a search engine is influenced not only by total energy consumption but also by how and when resources are allocated throughout the search process.
